...
class KAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, noise_scale=0.1, scale_base_mu=0.0, scale_base_sigma=1.0, base_fun=torch.nn.Sigmoid(), symbolic_enabled=True, bias_trainable=True, grid_eps=1.0, grid_range=[0, 1], sp_trainable=True, sb_trainable=True,
                 device='cpu', seed=0):
        '''
        ...
        for l in range(self.depth):
            ...
            # Ensure all values of the scaling factors are non-negative
            #scale_base = torch.abs(scale_base)
            scale_base = 0
    ...
    def initialize_from_another_model(self, another_model, x):
        ...
        for l in range(self.depth):
            ...
            preacts_sorted = preacts[sorted_indices]
            postsplines_sorted = postsplines[sorted_indices]
            
            coef = curve2coef_mono(preacts_sorted.reshape(batch, spb.size).permute(1, 0), postsplines_sorted.reshape(batch, spb.size).permute(1, 0), spb.grid, k=spb.k, device=self.device)

            coef_reordered = coef[torch.argsort(sorted_indices)]
            
            self.act_fun[l].coef.data = coef_reordered
    ...
    def forward(self, x):
        ...
        for l in range(self.depth):
            ...
            # print(f"x_numerical: {x_numerical}")
            # print(f"postacts in KAN: {postacts}")
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=0., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1e-3, stop_grid_update_step=50, batch=-1,
        ...
        def gradient_check(params, threshold=1e5): 
       # Check gradients for NaN, Inf, or extremely large values
           for param in params:
               if param.grad is not None:
                  if torch.isnan(param.grad).any():
                      print(f"NaN gradient detected in {param}")
                  if torch.isinf(param.grad).any():
                      print(f"Inf gradient detected in {param}")
                      inf_indices = torch.isinf(param.grad).nonzero(as_tuple=True)
                      print(f"Indices with Inf gradients: {inf_indices}")
                      print(f"Values with Inf gradients: {param.grad[inf_indices]}")
            # Check if the gradient has extremely large values 
                  max_grad_value = torch.max(torch.abs(param.grad))
                  if max_grad_value > threshold:
                      print(f"Exploding gradient detected in {param}!")
                      print(f"Maximum gradient value: {max_grad_value}")

        def closure():
            global train_loss, reg_
            optimizer.zero_grad()
            pred = self.forward(dataset['train_input'][train_id].to(self.device))
            if torch.isnan(torch.sum(pred, dim=1)).any():
                print("NaN detected in predictions (pred)")
                print(f"Predictions (pred): {pred}")
            if sglr_avoid == True:
            ...
            if torch.isnan(reg_).any():
                print("Nan in reg_")
            if torch.isnan(train_loss).any():
                print("Nan in train loss")
            objective = train_loss + lamb * reg_
            objective.backward()
            gradient_check(self.parameters())
            return objective.to(self.device)

...
def B_batch(x, grid, k=0, extend=True, device='cpu'):
    ...
    -----
        x : 2D torch.tensor
            inputs, shape (number of samples, number of splines)
    ...
    --------
        spline values : 3D torch.tensor
            shape (number of samples, number of splines, G+k). G: the number of grid intervals, k: spline order.
    '''
    x = x.unsqueeze(dim=2)
    ...
    if k == 0:
        ...
        print("grid spline shape:", grid.shape)
        print("x shape before B_batch:", x.shape)

        B_km1 = B_batch(x[:,:,0], grid=grid[0], k=k - 1)
        print("B_km1 shape:", B_km1.shape)
